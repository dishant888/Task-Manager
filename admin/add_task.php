...
if(isset($_REQUEST['submit']))
{
	...
	foreach ($_REQUEST['task'] as $tasks)
	{
		...
		mysqli_query($con,"insert into task_rows(task_id,team_id,title,created_by,estimated_hrs) values($last_id,$team_id,'$t',".$_SESSION['u_id'].",$estimated)");
	}
	$status = 'success';
	...
	<form action="add_task.php" method="post">
		<table class="table text-center" id="main_table">
			<tr>
				...
				<td>
					<select name="project" id="project" class="form-control-lg js-example-basic-single">
		...
		<tr class="main_tr">
			<th class="p-3">Task-Title:</th>
				...
				<td class="form-inline">
					<b>Estimated hrs:</b>
					<input type="number" step="0.01" name="estimated" class="form-control ml-2" required>
				</td>
				<td colspan="2">
...
<script type="text/javascript">
	...
	function rename(){
		...
		$('table#main_table tbody#main_tbody tr.main_tr').each(function() {
			...
			$(this).find('td:nth-child(3) select').attr({name: "task["+i+"][team_id]"});
			$(this).find('td:nth-child(4) input').attr({name: "task["+i+"][estimated]"});
			i++;
		});
	}
</script>